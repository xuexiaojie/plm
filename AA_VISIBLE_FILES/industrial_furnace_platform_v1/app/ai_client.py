    ...
    if not api_url or not api_key:
        return {
            ...
            "three_flows": {
                ...
                "information_flow": ["核对现场反馈、审图单、技术说明、图纸目录、材料表、专利等技术文档与计算输入之间的信息一致性"],
            },
            "risks": ["需核对现场反馈与计算假设是否一致", "需确认审图单意见是否已闭环", "需校验技术说明和材料表中的设备参数版本"],
            "conflicts": ["若图纸目录版本晚于计算输入，应重新复核计算边界条件"],
            "recommendations": ["将同一设备的计算、现场反馈、审图单、技术说明、图纸目录、材料表、专利等技术文档纳入同一分析包", "正式环境配置 AI_API_URL、AI_API_KEY、AI_MODEL 后调用大模型 API"],
        }
